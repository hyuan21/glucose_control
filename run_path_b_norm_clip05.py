"""
Path B + reward normalization + smaller grad clip (clip=0.5).

The previous Path B + normalize run saturated at |grad_s Q| = 10.0 every
step (clip too loose to be ineffective, too tight to let the critic
learn). Under normalized reward |Q| ~ 1-10 (vs ~1000 before), so the
appropriate clip is 1-2 orders of magnitude smaller.

We try clip=0.5: at this scale the per-row grad norm is at the same
scale as |Q| itself, mirroring the implicit clip-to-Q-scale ratio that
worked in the raw-reward run (clip=10 vs |Q|~1000 = ~1%).
...
params = {
    ...
    "use_path_b": True,
    "use_hjb": False,
    # Reduced from 10.0 to match normalized reward's |Q| ~ 1-10 scale.
    # Same relative scale as the original clip=10 vs |Q|~1000 setup.
    "path_b_grad_clip": 0.5,

    "normalize_reward": True,
}

print(f"Patient: adult#1   bas={bas:.4f}")
print(f"Mode:    QUICK PATH B + normalized + clip=0.5  (steps=50,000, seed=0)")
...
print("Run compare_quick.py to evaluate vs the normalized baseline.")
